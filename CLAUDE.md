...
2. **YouTube video → X + LinkedIn posts** — `python3 sns_helper.py "<url>"`.
...
- **Voice source of truth** = `linkedin_posts/IMPROVE_PROMPT.md` + the real posts in
  ...
  (`linkedin_posts/index.md` lists them).
- **Never invent facts** — names, numbers, prizes, quotes, links. Ask for the specifics
  ...
  `[TODO: ...]` markers. Generic filler prose is the failure mode, not blanks.
